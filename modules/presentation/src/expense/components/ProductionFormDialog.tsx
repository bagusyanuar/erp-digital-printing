...
interface ProductionFormDialogProps {
  isOpen: boolean;
  onClose: () => void;
  expense: ProductionExpenseMock | null;
  readOnly?: boolean;
}

export const ProductionFormDialog: React.FC<ProductionFormDialogProps> = ({
  isOpen,
  onClose,
  expense,
  readOnly = false,
  ...
  const [productCategoryId, setProductCategoryId] = useState(expense ? "prod-1" : "");
  const [supplierName, setSupplierName] = useState(expense ? expense.supplierName : "");
    ...
    <Dialog isOpen={isOpen} onClose={onClose} size="md" showCloseButton={false}>
      <form onSubmit={handleSubmit}>
        <CardHeader className="px-6 py-5 border-b border-border/50 flex flex-row items-start justify-between gap-4">
          <div className="space-y-0.5">
            <CardTitle variant="h4" weight="semibold" className="text-lg tracking-tight">
              {readOnly
                ? "Detail Pengeluaran Produksi"
                : expense
                ? "Ubah Pengeluaran Produksi"
                : "Tambah Pengeluaran Produksi"}
            </CardTitle>
            <CardDescription className="text-sm">
              {readOnly
                ? "Informasi rincian transaksi biaya produksi cetak."
                : "Catat biaya langsung yang terkait dengan proses produksi cetak."}
            </CardDescription>
          </div>
          <Button
            variant="ghost"
        ...
        <CardContent className="px-6 py-5 space-y-4">
              ...
              {!readOnly && (
                <ComboboxContent className="w-[var(--radix-popover-trigger-width)] bg-background border border-border/80 shadow-lg rounded-xl overflow-hidden z-[10000]">
                  ...
                  <ComboboxList className="max-h-60 overflow-y-auto p-1">
                    <ComboboxItem value="cat-1">ART PAPER</ComboboxItem>
                    <ComboboxItem value="cat-2">TINTA OUTDOOR</ComboboxItem>
                    <ComboboxItem value="cat-3">TONER / SPAREPART</ComboboxItem>
                    <ComboboxItem value="cat-4">STIKER</ComboboxItem>
                    <ComboboxItem value="cat-5">BAHAN</ComboboxItem>
                    <ComboboxItem value="cat-6">LAIN - LAIN</ComboboxItem>
                  </ComboboxList>
                </ComboboxContent>
              )}
            </Combobox>
          ...
          {/* Kategori Produk */}
          <div className="space-y-2">
            <Label className="text-sm font-semibold">Kategori Produk Terkait</Label>
            <Combobox value={productCategoryId} onValueChange={setProductCategoryId}>
              <ComboboxTrigger disabled={readOnly} className="font-semibold w-full h-11 border rounded-xl px-3 border-border/50 text-sm bg-background text-left flex items-center justify-between disabled:opacity-55 disabled:cursor-not-allowed">
                <span>
                  {productCategoryId === "prod-1" ? "Brosur & Flyer" : "-- Pilih Kategori Produk --"}
                </span>
              </ComboboxTrigger>
              {!readOnly && (
                <ComboboxContent className="w-[var(--radix-popover-trigger-width)] bg-background border border-border/80 shadow-lg rounded-xl overflow-hidden z-[10000]">
                  ...
                  <ComboboxList className="max-h-60 overflow-y-auto p-1">
                    <ComboboxItem value="prod-1">Brosur & Flyer</ComboboxItem>
                    <ComboboxItem value="prod-2">Banner / MMT</ComboboxItem>
                    <ComboboxItem value="prod-3">Merchandise & Acrylic</ComboboxItem>
                  </ComboboxList>
                </ComboboxContent>
              )}
            </Combobox>
          ...
          {/* Supplier & Sumber Dana */}
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className="text-sm font-semibold">Supplier / Vendor</Label>
              <TextField
                disabled={readOnly}
